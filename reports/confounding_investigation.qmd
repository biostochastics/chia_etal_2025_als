...
6. **Residual confounding in "robust" proteins**: Even the 22 tube-robust proteins predict tube type with AUC = 0.89
...
### Visualizing Country/Tube Effects in Tube-Robust Proteins

To demonstrate that even "tube-robust" proteins exhibit systematic country/tube effects, we examine the distribution of NPX values for ALL tube-robust proteins, comparing ALS vs. Healthy controls across Italy (HEPARIN) and US (EDTA) cohorts.

```{r tube-robust-protein-viz}
#| label: fig-tube-robust-viz
#| fig-cap: "All 'Tube-Robust' Proteins Show Strong Country/Tube Effects"
#| fig-width: 12
#| fig-height: 40
#| echo: false
...
# 1. Identify ALL tube-robust proteins (not just top 15).
...
# Combine tube-robust proteins with ALL original biomarkers to show the full picture
all_proteins_to_plot <- unique(c(tube_robust_top15_assays, original_biomarkers))

# 2. Prepare the data for plotting.
#    - Filter the main dataset to include tube-robust proteins AND all original biomarkers.
...
ggplot(plot_data_tube_effects, aes(x = Diagnosis_clean, y = NPX, fill = Diagnosis_Tube)) +
  ...
  labs(
    title = "Tube-Robust Proteins + All Original Biomarkers Show Country/Tube Effects",
    subtitle = "NPX distribution by diagnosis (ALS vs Healthy), combining tube-robust proteins and ALL 17 original biomarkers. P-values (lime green if p < 0.05, limma-adjusted for age & sex) and effect sizes (ES = logFC from limma, EDTA - HEPARIN) compare tube types within each diagnosis.\n★ = Original study biomarker from Figure 4a",
    x = "Diagnosis",
...
The visualization combines tube-robust proteins with **ALL 17 original biomarkers from the authors' Figure 4a** to demonstrate that **systematic tube type differences exist across both sets**. Even within healthy controls, we observe substantial EDTA vs. HEPARIN differences (shown as effect sizes and p-values). This pattern demonstrates that:
...
As demonstrated in [Figure @fig-tube-robust-viz], even "tube-robust" proteins show systematic differences between tube types within disease-free individuals. Here we quantify these effects statistically.
...
# Filter to only tube-robust proteins (same as old analysis)
...
#| tbl-cap: "Top tube-robust proteins with tube effects in disease-free individuals (limma-adjusted for age & sex)"
...
**Many tube-robust proteins differ by tube type even WITHOUT disease present!**
...
- `r sum(tube_effects_healthy$adj.P.Val < 0.05 & tube_effects_healthy$Assay %in% tube_robust_top15_assays)` of `r sum(tube_effects_healthy$Assay %in% tube_robust_top15_assays)` tube-robust proteins remain significant after FDR correction
...
2. **Healthy controls (50% significant):** Half of tube-robust proteins show tube effects without disease present
3. **Effect decomposition (median ratio = 0.22):** Tube effects are 20-50% as large as disease effects in tube-robust proteins
...
### A7. Why "Tube-Robust" Proteins Still Show Confounding {.unlisted}
...
### Pipeline Architecture {.unlisted}

```{r pipeline-dependency-graph}
#| label: fig-pipeline-graph
#| fig-cap: "Complete dependency graph showing all computational targets"
#| fig-width: 14
#| fig-height: 10
#| code-fold: true
#| cache: false
...
The dependency graph above shows the complete computational workflow. Each node represents a target (data object, model, or visualization), and edges show dependencies. Node size reflects execution time, while colors indicate target status.
...
- @fig-tube-robust-viz: Tube type robustness analysis
